...
public final class PolynomialRing<T> implements Ring<Polynomial<T>> {
    ...
    @Override
    public Polynomial<T> inverse(Polynomial<T> x) {
        Objects.requireNonNull(x, "Null found in inverse() - PolynomialRing");
        List<T> inverseList = x.getList().stream()
                .map(ring::inverse)
                .collect(Collectors.toList());

        return  Polynomial.from(inverseList);
    }
}